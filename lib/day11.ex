defmodule Aoc22.DayEleven do
  ...
  def throw_item(monkeys, from, to) do
    ...
    monkeys =
      List.update_at(monkeys, from, fn monkey -> %{monkey | items: tl(monkey[:items])} end)

    List.update_at(monkeys, to, fn monkey -> %{monkey | items: [item | monkey[:items]]} end)
  end
  ...
  def process_monkey(monkeys, n, %{items: [item | _items]} = monkey) do
    ...
    monkeys =
      List.update_at(monkeys, n, fn monkey ->
